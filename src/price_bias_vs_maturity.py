...
Effect of Varying T-t When mu = 0, r = 10%, sigma_t = 15%, xi = 1
Biais en fonction de S/X pour T-t = 45, 90, 135 Days
"""
...
for T_days, style in [(45, '-'), (90, '-.'), (135, ':')]:
    ...
    ax.plot(SX, bias_pct, linestyle=style, color='black', linewidth=2,
            label=f'T-t = {T_days} Days')
    print(f"  T-t = {T_days} Days termine ({time.time()-t0:.1f}s)")

ax.axhline(0, color='black', linewidth=0.5)
ax.axvline(1.0, color='black', linewidth=0.5)
ax.set_xlabel('S/X', fontsize=14)
ax.set_ylabel('PRICE BIAS (%)', fontsize=14)
ax.set_title(r'Effect of Varying $T - t$ When $\mu = 0$, $r = 10\%$, $\sigma_t = 15\%$, $\xi = 1$', fontsize=13)
ax.legend(fontsize=12)
ax.grid(True, alpha=0.3)
ax.set_xlim(0.88, 1.12)
ax.set_ylim(-5, 5)

filepath = os.path.join(output_dir, 'figure_4.png')
plt.tight_layout()
