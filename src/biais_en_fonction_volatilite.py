"""
Reproduction de la Figure 2 du papier
Effect of Varying sigma_t When mu = 0, r = 0, xi = 1, T-t = 180 Days
Biais en fonction de S/X pour sigma_0 = 10%, 15%, 20%
"""
...
for sigma0, style in [(0.10, '-'), (0.15, '--'), (0.20, ':')]:
    ...
    ax.plot(SX, bias_pct, linestyle=style, color='black', linewidth=2,
            label=r'$\sigma_t$' + f' = {sigma0*100:.0f}%')
    print(f"  sigma_0 = {sigma0*100:.0f}% termine ({time.time()-t0:.1f}s)")

ax.axhline(0, color='black', linewidth=0.5)
ax.axvline(1.0, color='black', linewidth=0.5)
ax.set_xlabel('S/X', fontsize=14)
ax.set_ylabel('PRICE BIAS (%)', fontsize=14)
ax.set_title(r'Effect of Varying $\sigma_t$ When $\mu = 0$, $r = 0$, $\xi = 1$, $T - t = 180$ Days', fontsize=13)
ax.legend(fontsize=12)
ax.grid(True, alpha=0.3)
ax.set_xlim(0.88, 1.12)
ax.set_ylim(-5, 5)

filepath = os.path.join(output_dir, 'figure_2.png')
plt.tight_layout()
